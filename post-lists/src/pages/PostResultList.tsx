...
const PostResultList = () => {
  ...
  const [getPostWithFilter] = useLazyQuery(GET_POST_WITH_FILTER_ADMIN);
  ...
  const queryPostWithFilters = async (filters: any) => {
    ...
    await getPostWithFilter({
      ...
      .then((result) => {
        ...
        const pList: PostWithFilter[] = returnedData?.listData.map(
          (post: PostWithFilter) => {
            return {
              id: post.id,
              title: post.title,
  ...
  useEffect(() => {
    ...
    if (searchParams.size > 0) {
      const filters: any = {
        page: 1,
        pageSize: 10000,
        approved: approvedState ? approvedState : "NOT_YET"
      };
            ...
            <ToggleGroupItem
              ...
              className={
                approvedState === EnumApprove.REJECT
                  ? "data-[state=on]:bg-red-700 data-[state=on]:text-white"
                  : ""
              }
            >
              <p>Đã huỷ</p>
